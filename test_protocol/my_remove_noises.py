...
# for megaface datasets from insightface , facescrub noise: 605(24), megaface noise: 707(707)
import os
import yaml
import numpy as np
import argparse
...
def remove_facescrub_noises(facescrub_noises_file, facescrub_feature_dir, facescrub_feature_outdir):
    ...
    Args:
        ...
        facescrub_feature_dir(str): the directory which contains the features of facescrub.
    ...
    for root, dirs, files in os.walk(facescrub_feature_dir):
        ...
        for feat_name in files:
            ...
            feat_name_ext = os.path.splitext(feat_name)[-1]  # 获取后缀名 .npy
    ...
    for (person_name, feat_name) in noises:
        ...
        center /= np.linalg.norm(center)  # L2归一化
    ...
    for root, dirs, files in os.walk(megaface_feature_dir):
        for feat_name in files:
            ...
            if feat_name_ext == '.npy':
                ...
                # print('cur_feature_outdir:', cur_feature_outdir)
                if not os.path.exists(cur_feature_outdir):
                    os.makedirs(cur_feature_outdir)
                cur_feature_outpath = os.path.join(cur_feature_outdir, feat_name)
                short_image_path = os.path.join(id1, id2, image_name)
                short_image_path = short_image_path.replace('\\', '/')
                # print('look',short_image_path)
                # return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_conf_file', type=str,
                        help = "The path of data_conf.yaml.", default='./data_conf.yaml')
    parser.add_argument('--remove_facescrub_noise', type=int, 
                        help = "Remove facescrub noise or not, 1 for remove.", default=1)
    parser.add_argument('--remove_megaface_noise', type=int,
                        help = "Remove megaface noise or not, 1 for remove.", default=1)
    parser.add_argument('--facescrub_feature_dir', type=str, default='/ssd2t/megaface_result/mv-softmax_ms1m_facescrub_feats')
    parser.add_argument('--facescrub_feature_outdir', type=str, default='/ssd2t/megaface_result/mv-softmax_ms1m_facescrub_feats_clean')
    parser.add_argument('--megaface_feature_dir', type=str, default='/ssd2t/megaface_result/mv-softmax_ms1m_megaface_feats')
    parser.add_argument('--megaface_feature_outdir', type=str, default='/ssd2t/megaface_result/mv-softmax_ms1m_megaface_feats_clean')
